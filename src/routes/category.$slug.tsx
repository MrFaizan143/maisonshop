...
export const Route = createFileRoute("/category/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          ...
          { name: "description", content: `Shop ${loaderData.category.name} at Maison. Cash on Delivery, free shipping over ₹499.` },
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-3xl px-4 py-24 text-center">
      ...
      <Link to="/" className="mt-6 inline-block font-mono text-[11px] uppercase tracking-[0.2em] underline">Back home</Link>
    </div>
  ),
});

const THEME_COPY: Record<ThemeKey, { eyebrow: string; tagline: string; descriptor: string }> = {
  fashion: { eyebrow: "Vol. 01 — Fashion", tagline: "An editorial of intent.", descriptor: "Asymmetric grids, oversized type, considered silhouettes." },
  grocery: { eyebrow: "Vol. 02 — Food", tagline: "From the soil, to the table.", descriptor: "Seasonal produce, slow pantry, ingredients with a story." },
  electronics: { eyebrow: "Vol. 03 — Electronics", tagline: "Sharp tools, sharper ideas.", descriptor: "Devices that disappear into the work." },
  home: { eyebrow: "Vol. 04 — Home", tagline: "Objects for a slower life.", descriptor: "Warm materials, soft edges, made to last." },
  beauty: { eyebrow: "Vol. 05 — Beauty", tagline: "Quiet rituals.", descriptor: "Formulations and tools for a thoughtful routine." },
};

function CategoryPage() {
  const { category } = Route.useLoaderData();
  const slug = category.slug as ThemeKey;
  const copy = THEME_COPY[slug] ?? { eyebrow: category.name, tagline: category.name, descriptor: "" };
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        ...
        <section className={cn(
          ...
          {category.image_url && (
            <div className="absolute inset-0">
              <img src={category.image_url} alt="" className="h-full w-full object-cover opacity-25" />
          ...
          <div className="relative mx-auto max-w-[1400px] px-5 sm:px-8 py-20 sm:py-28">
            ...
            <motion.h1
              ...
              className={cn(
                ...
                !isFashion && !isFood && "font-display text-[clamp(3rem,8vw,7rem)] font-medium tracking-tight",
              )}
            >
              {copy.tagline}
            </motion.h1>
        ...
        {/* ===== Toolbar ===== */}
        <div className="sticky top-16 z-20 border-b border-border bg-background/85 backdrop-blur">
          <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-4 px-5 sm:px-8 py-4">
            ...
            <div className="flex items-center gap-3">
              <label className="font-mono text-[11px] uppercase tracking-[0.2em] text-muted-foreground hidden sm:inline">Sort</label>
              <select
                value={sort}
  ...
  return (
    <div className="grid grid-cols-12 gap-4 md:gap-6">
      {products.map((p, i) => {
        ...
        const pattern = i % 6;
        const span =
          pattern === 0 ? "col-span-12 md:col-span-7" :
          pattern === 1 ? "col-span-6 md:col-span-5" :
          pattern === 2 ? "col-span-6 md:col-span-4" :
          pattern === 3 ? "col-span-6 md:col-span-4" :
          pattern === 4 ? "col-span-12 md:col-span-4" :
          "col-span-12 md:col-span-8";
        const aspect =
          pattern === 0 || pattern === 5 ? "aspect-[16/10]" :
          ...
          >
            ...
            >
              <div className={cn("relative overflow-hidden bg-muted", aspect)}>
                {p.image_url ? (
                  <img src={p.image_url} alt={p.title} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.04]" />
              ...
              <div className="mt-4 flex items-baseline justify-between gap-4 border-t border-border pt-3">
                ...
                <div className="font-mono text-sm whitespace-nowrap">₹{p.price.toLocaleString("en-IN")}</div>
              </div>
            </Link>
          </motion.div>
        );
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {products.map((p, i) => (
        ...
        >
          <Link to="/product/$slug" params={{ slug: p.slug }} className="group block overflow-hidden rounded-[2rem] bg-card border border-border shadow-card hover:shadow-pop transition-all">
            <div className="relative aspect-[4/3] overflow-hidden rounded-b-[2rem] bg-muted">
              {p.image_url ? (
                <img src={p.image_url} alt={p.title} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.06]" />
              ) : <div className="grid h-full place-items-center text-muted-foreground text-xs">No image</div>}
            </div>
            <div className="p-6">
              <div className="font-mono text-[10px] uppercase tracking-[0.25em] text-muted-foreground">— Pantry</div>
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <div className="rounded-full bg-accent text-accent-foreground px-4 py-2 text-[11px] font-mono uppercase tracking-[0.2em]">Add</div>
              </div>
            </div>
          </Link>
        </motion.div>
